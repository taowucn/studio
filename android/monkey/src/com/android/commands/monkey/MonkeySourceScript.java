...
public class MonkeySourceScript implements MonkeyEventSource {
    ...
    private static final String EVENT_KEYWORD_ACTIVITY_WITH_MEDIA_ARGS = "LaunchWithMediaArgs";
    ...
    private void handleEvent(String s, String[] args) {
        ...
        if (s.indexOf(EVENT_KEYWORD_ACTIVITY_WITH_MEDIA_ARGS) >= 0 && args.length >= 2) {
            String pkg_name = args[0];
            String cl_name = args[1];
	     String mediaArgs = args[2];
            long alarmTime = 0;
	    ...
	    if (args.length > 3) {
                try {
                    alarmTime = Long.parseLong(args[3]);
                } catch (NumberFormatException e) {
                    System.err.println("// " + e.toString());
                    return;
                }
            ...
            if (args.length == 3) {
                MonkeyActivityEvent e = new MonkeyActivityEvent(mApp, mediaArgs, 0);
                mQ.addLast(e);
            } else {
                MonkeyActivityEvent e = new MonkeyActivityEvent(mApp, mediaArgs, alarmTime);
                mQ.addLast(e);
            }
            return;
        }
    ...
    public MonkeyEvent getNextEvent() {
        ...
        MonkeyEvent ev;
	System.out.println("++MonkeySourceScript/getNextEvent - 00000000000000");
        if (mQ.isEmpty()) {
            try {
                readNextBatch();
		System.out.println("++MonkeySourceScript/getNextEvent - 1");
            } catch (IOException e) {
                return null;
            }
        }
